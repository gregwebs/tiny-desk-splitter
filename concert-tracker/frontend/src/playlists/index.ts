...
// between window.Playlists / window.Player and the widget's typed Ports.
//
// The add panel lives in #sidebar-add-section — a page-lifetime sibling of
// #player-sidebar outside the player widget's render root, so player re-renders
// don't wipe the add-panel runtime. Unlike the splitter — mounted and disposed
// per toggle — this widget is mounted once on first openAdd and kept for the
// page lifetime; open/close flows over its Ports.
...
function tearDownChrome(): void {
  document.body.classList.remove("showing-add");
...
// showing, and tell the widget to reset. Both flags live on body now so the
// observer simply checks: showing-add present AND sidebar-open just removed.
...
function openAdd(target: AddTarget): Promise<void> {
  ...
  // show the add-panel overlay.
  window.Player?.openSidebar();
  document.body.classList.add("showing-add");
