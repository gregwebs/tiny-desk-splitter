//! Dependency direction between `db::connection`, concert reads, and `events`
//! (pinned down here per #63, before #64 moves concert reads into their own
//! module): `db::connection::run_migrations` calls `events::backfill`, which
//! reads concerts via `crate::db::list_concerts` (still defined in this
//! module). So `events` may depend on concert read operations, and
//! `db::connection` may depend on `events` — but concert read operations must
//! never depend back on `db::connection` internals or on `events`, or the
//! migration startup path forms a cycle.
...
// refactor, issue #69): re-exports so existing `db::...` call sites keep
// compiling unchanged while callers migrate to domain paths in #66/#67.
// Removed by #68.
...
pub mod tests {
    ...
    /// `pub(crate)`: shared with `db::connection`'s tests, which need a seeded
    /// concert row to exercise migration/backfill re-runs.
    ...
    /// `pub(crate)`: shared with `db::connection`'s audit-timestamp tests.
    pub(crate) fn concert_ts(conn: &Connection, id: i64, column: &str) -> Option<String> {
        conn.query_row(
            &format!("SELECT {} FROM concerts WHERE id = ?1", column),
            params![id],
            |row| row.get(0),
