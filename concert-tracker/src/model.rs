...
// Filesystem-backed media facts (downloaded source, track, and interlude
// lookup; source redundancy) live in `concert_media`. Re-exported here for
// compatibility with existing call sites; touched code should import from
// `concert_media` directly. See docs/change/2026-07-09-concert-media-inventory.md.
pub use crate::concert_media::{
    find_downloaded_file, find_interlude_file, find_interlude_track_file, find_track_file,
    is_video_extension, source_redundant, INTERLUDE_EXTENSIONS,
};
...
// Reconstruction-item construction (`build_reconstruction`) lives in
// `concert_media` — it decides which songs/interludes are playable from
// filesystem facts, but returns `PlaybackItem` (defined here) since that type
// is shared with `playback`. Re-exported for compatibility.
pub use crate::concert_media::build_reconstruction;
...
// `list_tracks` / `list_all_tracks` (filesystem-backed) live in
// `concert_media`, alongside the private `track_file_extension` helper they
// share with `list_all_track_details`. Re-exported for compatibility.
pub use crate::concert_media::{list_all_track_details, list_all_tracks, list_tracks};
